#!/usr/bin/env python3
# Local mirror of the "Pre-clone KDE Craft and apply local patches" step
# from .github/workflows/windows-arm64-release.yml. Same logic, same
# fail-fast guards. This file is NOT committed to the repo — it only
# lives in the workspace so the two patches can be reproduced on demand
# when craft-clone is re-cloned.
import sys
import re
...
# the cd. Modern Windows cmd.exe does not search the current directory
# for executables by default, so the recipe fails with "'miniperl' is
# not recognized" even though miniperl.exe is right there. Put the perl
# source root on PATH so the bare name resolves.
old_globenv = (
    '    def _globEnv(self):\n'
    '        env = {}\n'
...
# Patch 6: libs/openssl/openssl.py — the upstream blueprint only knows
# about x86_64 (VC-WIN64A) and falls back to VC-WIN32 for everything
# else, which on a Windows ARM64 host produces a build configured for
# 32-bit x86 (the C sources get compiled with -DAES_ASM/-DOPENSSL_IA32_SSE2
# /etc. and Configure emits nasm rules for x86 .asm files). OpenSSL ships
# a proper VC-WIN64-ARM target that uses the ARM64 .S perlasm modules, so
# select it for arm64.
...
# Patch 7: libs/liblzma/liblzma.py — xz 5.2.3's windows/ project files only
# declare Win32, x64, and ReleaseMT|x64 platform configurations. On a
# Windows ARM64 host MSBuild rejects /p:Platform=arm64 with MSB4126 "The
# specified solution configuration 'Release|arm64' is invalid". xz itself
# is pure C with no x86-specific assembly, so cloning the x64 sections of
# xz_win.sln, liblzma.vcxproj, and liblzma_dll.vcxproj as arm64 siblings
# is sufficient. Inject a configure() override into PackageMSBuild that
# does the cloning right before MSBuild runs.
...
# Patch 8: libs/libunistring/libunistring.py — MSYS GNU windres (binutils
# 2.46) does not support pe-arm64-little. libunistring's build compiles
# lib/libunistring.rc into a COFF resource object and links it into
# libunistring.dll; windres produces x64 COFF, and the ARM64 linker
# rejects it with LNK1112. Fix: inject a configure() override into
# Package that installs a thin windres wrapper at /usr/local/bin/windres
# (the first directory in the MSYS autotools PATH). The wrapper calls
# the real windres unchanged and then patches bytes 0-1 of the COFF
# output from AMD64 (0x8664) to ARM64 (0xAA64). Safe because .res.obj
# files contain only resource data, not machine code.
...
# Patch 9: libs/libffi/libffi.py — AutoToolsBuildSystem hardcodes
# --host=x86_64-w64-mingw32 for all non-x86_32 Windows builds, which
# causes libffi's configure.ac to select the src/x86/win64 backend
# instead of src/aarch64 (ARM_WIN64). Override self.platform to force
# aarch64-w64-mingw32 for ARM64 hosts. Also add -marm64 to CCAS so
# msvcc.sh generates ARM64 assembly objects instead of x64 COFF
# (otherwise LNK1112 "machine type 'x64' conflicts with ARM64").
...
new_libffi_arch = (
    ...
    '                # msvcc.sh needs -marm64 to generate ARM64 assembly/objects.\n'
    '                # Without this flag msvcc.sh defaults to AMD64 and produces x64\n'
    '                # COFF objects that the ARM64 linker rejects with LNK1112.\n'
    '                arch = " -marm64"\n'
    '            self.subinfo.options.configure.args += [f"CCAS={wrapper}{arch}"]\n'
)
...
new_libffi_tail = (
    ...
    '\n'
    '        if CraftCore.compiler.isWindows and CraftCore.compiler.architecture == CraftCompiler.Architecture.arm64:\n'
    '            # AutoToolsBuildSystem hardcodes x86_64-w64-mingw32 for all non-x86_32\n'
    '            # Windows builds. Override it so libffi\'s configure.ac selects the\n'
    '            # src/aarch64 backend (TARGET=ARM_WIN64) instead of src/x86/win64.\n'
...
if old_libffi_tail not in s:
    print('ERROR: could not find CFFI_BUILDING_DLL tail block in libffi.py', file=sys.stderr)
    sys.exit(1)
...
# Patch 10: libs/python/python.py — Python's MSBuild produces ARM64
# artifacts at PCbuild/arm64/ not PCbuild/amd64/. The upstream blueprint
# hardcodes PCbuild/amd64/ in install(), so on ARM64 none of the
# executables, DLLs, or .pyd extension modules are found.
...
new_python_install = (
    ...
    '            verMinor = self.subinfo.buildTarget.split(".")[1]\n'
    '            debugSuffix = "_d" if self.buildType() == "Debug" else ""\n'
    '            # PCbuild output dir depends on target architecture\n'
    '            from CraftCompiler import CraftCompiler\n'
...
# Replace all remaining PCbuild/amd64/ references in install() with pcbuildArch
old_pcbuild_refs = [
...
for old_ref, new_ref in old_pcbuild_refs:
    if old_ref not in s:
        print(f'ERROR: could not find PCbuild/amd64 reference in python.py: {old_ref!r}', file=sys.stderr)
        sys.exit(1)
...
# Patch 11: libs/libjpeg-turbo/libjpeg-turbo.py — NASM only produces
# x86/x86_64 COFF objects. Linking them into an ARM64 DLL causes
# LNK1112 "machine type 'x64' conflicts with ARM64". Disable SIMD
# entirely on Windows ARM64; libjpeg-turbo falls back to portable C.
libjpeg_path = sys.argv[10]
...
if old_libjpeg_imports not in s:
    print('ERROR: could not find import block in libjpeg-turbo.py', file=sys.stderr)
    sys.exit(1)
s = s.replace(old_libjpeg_imports, new_libjpeg_imports)
old_libjpeg_pkg = (
...
new_libjpeg_pkg = (
    ...
    '            self.subinfo.options.configure.args += ["-DENABLE_SHARED=ON", "-DENABLE_STATIC=OFF"]\n'
    '        if CraftCore.compiler.isWindows and CraftCore.compiler.architecture == CraftCompiler.Architecture.arm64:\n'
    '            # NASM only produces x86/x86_64 COFF objects; linking them into an\n'
    '            # ARM64 DLL causes LNK1112 "machine type \'x64\' conflicts with ARM64".\n'
    '            # Disable SIMD entirely — libjpeg-turbo falls back to portable C code.\n'
    '            self.subinfo.options.configure.args += ["-DWITH_SIMD=OFF"]\n'
)
if old_libjpeg_pkg not in s:
    print('ERROR: could not find Package.__init__ block in libjpeg-turbo.py', file=sys.stderr)
    sys.exit(1)
with open(libjpeg_path, 'w', encoding='utf-8', newline='') as f:
    f.write(s.replace(old_libjpeg_pkg, new_libjpeg_pkg))
print(f'Patched {libjpeg_path}: -DWITH_SIMD=OFF for ARM64 Windows (avoids NASM LNK1112)')

# Patch 12: libs/pixman/pixman.py — pixman builds x86 MMX/SSE2/SSSE3
# SIMD paths via MSVC intrinsics that are x86-only (mmintrin.h, etc.) and
# an ARM64 NEON path via GAS .S assembly that MSVC cannot assemble.
# All four SIMD paths fail to build with MSVC on Windows ARM64.
# Disable them all; pixman falls back to portable C.
pixman_path = sys.argv[11]
...
new_pixman_pkg = (
    ...
    '        from CraftCompiler import CraftCompiler\n'
    '        if CraftCore.compiler.architecture == CraftCompiler.Architecture.arm64:\n'
    '            # macOS and Windows ARM64: no assembler for pixman-arma64-neon-asm.S,\n'
    '            # and MSVC mmintrin.h/simd headers require x86 — disable all x86+NEON SIMD.\n'
...
if old_pixman_pkg not in s:
    print('ERROR: could not find Package.__init__ block in pixman.py', file=sys.stderr)
    sys.exit(1)
with open(pixman_path, 'w', encoding='utf-8', newline='') as f:
    f.write(s.replace(old_pixman_pkg, new_pixman_pkg))
print(f'Patched {pixman_path}: disable a64-neon/mmx/sse2/ssse3 SIMD for ARM64 Windows')
